...
class BaseOptimizer(object):
    ...
    def optimize_clf(self, population: int = 10, generations: int = 3,
        ...
        for i in range(len(hof)):
            ...
            self.tracker.optimization_logger.info("Individual TOP {}".format(i + 1))
            self.tracker.optimization_logger.info("Individual accuracy: {}".format(best_score))
            self.tracker.optimization_logger.info("Best classifier: {}".format(str(self.get_clf(hof[i]))))
            self.tracker.optimization_logger.info("Hyperparams: {}".format(str(self.get_clf(hof[i]).get_params())))
    ...
    def custom_ea_simple(self, population, toolbox, logbook,
        ...
        for gen in range(start_gen, ngen + 1):
            ...
            for i in range(len(halloffame[:2])):
                ...
                self.tracker.optimization_logger.info("Individual TOP {}".format(i + 1))
                self.tracker.optimization_logger.info("Individual accuracy: {}".format(best_score))
                self.tracker.optimization_logger.info("Best classifier: {}".format(str(self.get_clf(halloffame[i]))))
                self.tracker.optimization_logger.info(
