...
def test_optimizer(clf_class, dataset):
    X, y = dataset(return_X_y=True)
    evolvable_hyperparams = HyperparameterSpace.get_default_hyperparameter_space(clf_class)
    opt = SklearnOptimizer(features=X, labels=y, fitness_score="accuracy", clf_class=clf_class,
                           hyperparam_space=evolvable_hyperparams)
...
@pytest.mark.parametrize('dataset',
                         (load_breast_cancer, load_iris))
def test_optimizer_use_parallel(clf_class, dataset):
    X, y = dataset(return_X_y=True)
    evolvable_hyperparams = HyperparameterSpace.get_default_hyperparameter_space(clf_class)
    my_seed = 25
    population = 50
    ...
    opt_with_parallel = SklearnOptimizer(features=X, labels=y, fitness_score="accuracy", seed=my_seed,
                                         use_parallel=True,
    ...
    opt = SklearnOptimizer(features=X, labels=y, fitness_score="accuracy", seed=my_seed, use_parallel=False,
                           hyperparam_space=evolvable_hyperparams, clf_class=clf_class)
...
def test_reproducibility(clf_class, target_score):
    X, y = load_iris(return_X_y=True)
    evolvable_hyperparams = HyperparameterSpace.get_default_hyperparameter_space(clf_class)
    population = 2
    generations = 2
    seed = 25
    distinct_seed = 2
    optimizer1 = SklearnOptimizer(features=X, labels=y, fitness_score="accuracy",
    ...
    optimizer2 = SklearnOptimizer(features=X, labels=y, fitness_score="accuracy",
                                  eval_function=target_score, seed=seed, clf_class=clf_class,
                                  hyperparam_space=evolvable_hyperparams)
    result2 = optimizer2.optimize_clf(population=population, generations=generations)
    optimizer3 = SklearnOptimizer(features=X, labels=y, fitness_score="accuracy",
